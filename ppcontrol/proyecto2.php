                        ...
                        <div class="col-md-12">
                            <div class="card card-plain">
								...
								<td>
								    ...
								    <h4 class="title">Proyectos Museo del Agua</h4>
...
<div class = "col-lg-12 well" style = "margin-top:10px;">
					...
					<div id = "book_table">
						<table id = "table" class = "table table-bordered table-hover">
							...
							<tbody>
								<?php
									$qbook = $link->query("SELECT id_proyecto, tit_proyecto, des_proyecto, fec_proyecto, dur_proyecto, obs_proyecto, est_proyecto, tie_proyecto FROM proyecto") or die(mysqli_error());
									while($fbook = $qbook->fetch_array()){
										
								?>
								<tr>
									<td><?php echo $fbook['tit_proyecto']?></td>
									<td><?php echo $fbook['des_proyecto']?></td>
									<td><?php echo $fbook['obs_proyecto']?></td>
									<td><?php echo date("m-d-Y", strtotime($fbook['fec_proyecto']))?></td>
									<td><?php echo $fbook['dur_proyecto']?>-<?php echo $fbook['tie_proyecto']?></td>
									<td><a class = "btn btn-danger delbook_id" value = "<?php echo $fbook['id_proyecto']?>"><span class = "glyphicon glyphicon-remove"></span> Eliminar</a> <a value = "<?php echo $fbook['id_proyecto']?>" class = "btn btn-warning ebook_id"><span class = "glyphicon glyphicon-edit"></span> Editar</a></td>
								</tr>
								<?php
									}
								?>
						...
						<div class = "col-lg-6">
							<form method = "POST" action = "save_proyecto_query.php" enctype = "multipart/form-data">
								...
								<div class = "form-group">
									...
									<input type = "date" name = "fec_proyecto" required = "required" class = "form-control" />
								</div>
								<div class = "form-group">
...
<td>
<input type = "number" min = "0" name = "dur_proyecto" required = "required" class = "form-control" />
</td>
...
<select case class="form-control" data-style="btn btn-primary btn-round" data-size="7" name="tie_proyecto" id="tie_proyecto">
<option value="Seleccionar" selected="selected">Tiempo</option>
	...
	<script type = "text/javascript">
		$(document).ready(function(){
			...
			$('.delbook_id').click(function(){
				...
				setTimeout(function(){
					window.location = 'delete_proyecto.php?book_id=' + $book_id;
				}, 1000);
			});
			$('.ebook_id').click(function(){
				...
				$('#book_table').fadeOut();
				$('#add_book').hide();
				$('#edit_form').load('load_proyecto.php?book_id=' + $book_id);
			});
		});
	</script>
