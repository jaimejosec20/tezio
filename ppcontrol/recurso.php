    ...
    -->
            ...
            <div class="sidebar-wrapper">
                <ul class="nav">
                    ...
                    <li  class="active">
                        <a href="recurso.php">
                            <img src="images/recursosb.png" width="30" height="30">
                            &nbsp;&nbsp;&nbsp;Recursos
                        </a>
                    </li>
                    <li>
                        ...
                        <div class="col-md-12">
                            <div class="card card-plain">
								...
								<td width="94%">
								    ...
								    <h4 class="title">Recursos Museo del Agua</h4>
...
<div class = "col-lg-12 well" style = "margin-top:10px;">
					...
					<div id = "book_table">
						<table id = "table" class = "table table-bordered table-hover">
							...
							<tbody>
								<?php
									$qbook = $link->query("SELECT recurso.id_recurso, recurso.des_recurso, recurso.obs_recurso, recurso.fec_recurso, recurso.id_tiprecurso, recurso.est_recurso, tiprecurso.des_tiprecurso FROM recurso INNER JOIN tiprecurso ON recurso.id_tiprecurso=tiprecurso.id_tiprecurso") or die(mysqli_error());
									while($fbook = $qbook->fetch_array()){
										
								?>
								<tr>
									<td><?php echo $fbook['des_recurso']?></td>
									<td><?php echo $fbook['obs_recurso']?></td>
									<td><?php echo date("d-m-Y", strtotime($fbook['fec_recurso']))?></td>
									<td><?php echo $fbook['des_tiprecurso']?></td>
									<td><a class = "btn btn-danger delbook_id" value = "<?php echo $fbook['id_recurso']?>"><span class = "glyphicon glyphicon-remove"></span> Eliminar</a> <a value = "<?php echo $fbook['id_recurso']?>" class = "btn btn-warning ebook_id"><span class = "glyphicon glyphicon-edit"></span> Editar</a></td>
								</tr>
								<?php
									}
								?>
					...
					<div id = "book_form" style = "display:none;">
						...
						<div class = "col-lg-6">
							<form method = "POST" action = "save_recurso_query.php" enctype = "multipart/form-data">
								<div class = "form-group">
									<label>Descripción del Recurso:</label>
									<input type = "text" name = "des_recurso" required = "required" class = "form-control" />
								</div>
								<div class = "form-group">
									<label>Observación del Recurso:</label>
									<input type = "text" name = "obs_recurso" class = "form-control" />
								</div>
								<div class = "form-group">
									<label>Fecha del Recurso:</label>
									<input type = "date" name = "fec_recurso" class = "form-control" required = "required"/>
								</div>
								<div class = "form-group">
					...
					<select class='form-control' name='trecurso' id='trecurso' required>
						<option value="">Seleccione...</option>
							...
							while($rw=mysqli_fetch_array($query_trecurso))	{
								?>
							<option value="<?php echo $rw['id_tiprecurso'];?>"><?php echo $rw['des_tiprecurso'];?></option>			
								<?php
							}
							?>
					</select>	
								...
								<div class = "form-group">
									<button name = "save_recurso" class = "btn btn-primary"><span class = "glyphicon glyphicon-save"></span> Enviar</button>
								</div>
							</form>		
						</div>	
					</div>
	...
	<script type = "text/javascript">
		$(document).ready(function(){
			...
			$('.delbook_id').click(function(){
				...
				setTimeout(function(){
					window.location = 'delete_recurso.php?book_id=' + $book_id;
				}, 1000);
			});
			$('.ebook_id').click(function(){
				...
				$('#book_table').fadeOut();
				$('#add_book').hide();
				$('#edit_form').load('load_recurso.php?book_id=' + $book_id);
			});
		});
	</script>
